...
import { getOfferItem, updateOfferItem } from "../../api/apiOfferItems";
...
const EditOfferItemPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchOfferItem = async () => {
            try {
                const item = await getOfferItem(offerItemId);
                ...
                if (isMounted) {
                    setLoadingError("Nie udało się pobrać danych usługi. Spróbuj ponownie później.");
                }
            } finally {
                if (isMounted) {
                    setIsLoading(false);
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            await updateOfferItem(offerItemId, request);
            navigate(`/admin/offer/${offerItemId}`, {
                replace: true,
                state: {
                    successMessage: "Dane usługi zostały zaktualizowane.",
                },
            });
        } catch {
        ...
        <section className="mx-auto w-full max-w-7xl px-6 py-10 sm:px-10 sm:py-14 lg:px-16 lg:py-16">
            <div className="mb-10 max-w-2xl sm:mb-12">
                ...
                <p className="mt-3 text-sm leading-6 text-white/60 sm:text-base">
                    Zaktualizuj informacje widoczne w ofercie.
                </p>
            </div>
            ...
            {isLoading && (
                <div
                    role="status"
                    className="
                        flex min-h-52 max-w-4xl flex-col items-center justify-center rounded-4xl border
            ...
            {!isLoading && loadingError && (
                ...
                >
                    ...
                    <Link
                        to={`/admin/offer/${offerItemId}`}
                        className="
                            mt-6 flex items-center gap-2 rounded-full border border-[var(--gold)]/60 px-5 py-2.5
                            text-sm font-semibold text-[var(--gold)] focus-visible:outline-none focus-visible:ring-2
            ...
            {!isLoading && !loadingError && (
                ...
                >
                    ...
                    ">
                        <button
                            type="button"
                            onClick={() => navigate(`/admin/offer/${offerItemId}`)}
                            disabled={isSubmitting}
                            className="
                                flex cursor-pointer items-center justify-center gap-2 rounded-full border
                                border-[var(--gold)]/55 px-6 py-3 text-sm font-semibold text-[var(--gold)]
                                hover:bg-[var(--gold)]/10 focus-visible:outline-none focus-visible:ring-2
                                focus-visible:ring-[var(--gold)] disabled:opacity-50
                            "
                        >
                            <ArrowLeft aria-hidden="true" size={17} />
                            Anuluj
                        ...
                        <button
                            ...
                            className="
                                ...
                                hover:bg-transparent hover:text-[var(--gold)] focus-visible:outline-none
                                focus-visible:ring-2 focus-visible:ring-[var(--gold)] disabled:opacity-50
                            "
                        >
                            {isSubmitting ? (
                                <LoaderCircle
